...
$$
\vec{F}_{mag} = Q(\vec{v} \times \vec{B}) \tag{5.1}
$$
...
$$
\vec{F} = q[ \vec{E} + ( \vec{V} \times \vec{B} ) ] \tag{5.2}
$$
...
$$
QvB = m \frac{v^2}{R} \quad \text{ or } \quad p = Q B R \tag{5.3}
$$
...
$$
\mathrm{d}ot{y} = \omega \dot{z} \qquad \mathrm{d}ot{z} = \omega \left( \frac{E}{B}  - \dot{y} \right) \tag{5.5}
$$
...
$$
y(t) = \frac{E}{\omega B} (\omega t - \sin \omega t) \qquad z(t) = \frac{E}{\omega B} (1 - \cos \omega t) \tag{5.7}
$$
...
$$
\vec{F}_B = \int (\vec{v} \times \vec{B}) \mathrm{d} q = \int (\vec{v} \times \vec{B}) \lambda \mathrm{d} l = \int (\vec{I} \times \vec{B}) \mathrm{d} l \tag{5.15}
$$

Inasmuch as __I__ and dl both point in the same direction, we can just as well write this as

$$
\vec{F}_B = \int I ( \mathrm{d} \vec{l} \times \vec{B}) \tag{5.16}
$$

Typically, the current is constant (in magnitude) along the wire, and in that case, I comes outside the integral:

$$
\vec{F}_B = I \int ( \mathrm{d} \vec{l} \times \vec{B}) \tag{5.17}
$$
...
$$
W_{mag} = F_{mag} h = I B a h \tag{5.19}
$$
...
$$
F_{vert} = \lambda a w B = I B a \tag{5.20}
$$
...
$$
\vec{K} = \frac{\mathrm{d} \vec{I}}{\mathrm{d} l _{\perp}} \tag{5.22} 
$$
...
$$
\vec{F}_{mag} = \int ( \vec{v} \times \vec{B}) \sigma \mathrm{d} a = \int (\vec{K} \times \vec{B}) \mathrm{d} a \tag{5.24}
$$
...
$$
\vec{F}_{mag} = \int (\vec{v} \times \vec{B}) \rho \mathrm{d} \tau = \int (\vec{J} \times \vec{B} ) \mathrm{d} \tau \tag{5.27}
$$
...
Because __J__ varies with _s_, we must integrate $ \eqref{5.25} $. The current through the shaded patch (Fig 5.16) is $ J \mathrm{d} a_{\perp} $, and $ \mathrm{d} a_{\perp} = s \mathrm{d} s \mathrm{d} \phi $ so
...
$$
I = \int_S J \mathrm{d} a_{\perp} = \int_S \vec{J} \cdot \mathrm{d} \vec{a} \tag{5.28}
$$
...
$$
\nabla \cdot  \vec{J} = - \frac{\partial \rho}{\partial t} \tag{5.29}
$$
...
$$
\sum_{i = 1} ^n ( ) q_i \vec{v}_i \sim \int_{line} () \vec{I} \mathrm{d} l \sim \int_{surf} ( ) \vec{K} \mathrm{d} a \sim \int_{vol} ( ) \vec{J} \mathrm{d} \tau \tag{5.30}
$$
