...
The simplest method is to chop the line into symmetrically placed pairs (at $ \pm x $), quote the result of Ex. 2.1 (with $ d/2 \to x$, $q \to \lambda dx$), and integrate ($x: 0\to L$). But here's a more general approach:

$$
\vec{r} = z\hat{z}, \qquad \vec{r}' = x\hat{x}, \qquad dl' = dx; \\
\Delta \vec{r} = \vec{r} - \vec{r}' = z\hat{z} - x\hat{x}, \qquad \Delta r = \sqrt{z^2 + x^2}, \qquad \hat{\Delta \vec{r}} = \frac{\vec{\Delta \vec{r}}}{\Delta r} = \frac{z\hat{z}-x\hat{x}}{\sqrt{z^2 + x^2}}. \\
$$


\begin{align*}
\vec{E} &= \frac{1}{4\pi\epsilon_0}\int_{-L}^{L} \frac{\lambda}{z^2 + x^2}\frac{z\hat{z} - x\hat{x}}{\sqrt{z^2 + x^2}}dx \\
&= \frac{\lambda}{4\pi\epsilon_0}\left[z\hat{z}\int_{-L}^{L}\frac{1}{(z^2 + x^2)^{3/2}}dx - \hat{x}\int_{-L}^{L}\frac{x}{(z^2+ x^2)^{3/2}}dx \right] \\
&= \frac{\lambda}{4\pi\epsilon_0}\left[z\hat{z}\left(\frac{x}{z^2\sqrt{z^2 + x^2}}\right)\bigg|_{-L}^{L} - \hat{x}\left(-\frac{1}{\sqrt{z^2 + x^2}}\right)\bigg|_{-L}^{L}\right] \\
&= \frac{1}{4\pi\epsilon_0}\frac{2\lambda L}{z\sqrt{z^2 + L^2}}\hat{z}
\end{align*}

For points far from the line ($ z \gg L $),

$$
E \approx \frac{1}{4\pi\epsilon_0}\frac{2\lambda L}{z^2}.
$$

This makes sense: From far awa the line looks like a point charge $ q = 2\lambda L $. In the limit $ L \to \infty $, on the other hand, we obtain the field of an infinite straight wire: 
