from electrodynamics.electrodynamics import edit_book

def test_edit_book():

    test_text = """# 1.2: Differential Calculus
...
In words: If we increment $x$ by an infinitesimal amount $ \mathrm{d}{x} $, then $ f $ changes by an amount $ \mathrm{d}{f} $; the derivative is the proportionality factor. Foe example, in Fig. 1.17(a), the function varies slowly with $x$, and the derivative is correspondingly small. In Fig 1.17(b), $f$ increases rapidly with $x$, and the derivative is large as you move away from $ x = 0 $.
_Geometrical interpretation_: The derivative $ \frac{\mathrm{d} f}{\mathrm{d} x} $ is the slope of the graph of $f$ versus $x$.
...
_Geometrical interpretation of the Gradient_: Like any vector, the gradient has _magnitude_ and _direction_. To determine its geometrical meaning, let's rewrite the dot product using {eq}`eq1.1`
...
(For once, I write the unit vectors to the left, just so no one will think that this means $ \partial{\hat{x}}/\partial x $ and so on, which would be zero since the coordinate directions are constant.) The term in parentheses is called __del__:
